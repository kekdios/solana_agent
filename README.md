...
1. Add your API key to `.env` (default chat provider is **NanoGPT**):
   ```
...
**Testing exec sandbox:** From the project root run `node scripts/test-exec-sandbox.js` to run the exec/sandbox test suite (temp workspace, echo, cwd, workdir, timeout, script run, config).
