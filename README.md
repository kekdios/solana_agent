...
**"Server failed to start" in the built app:** The server runs inside Electron (Method 2). If it fails, the error screen shows the **Error** and **Stack** (e.g. missing dependency or bad path). Ensure you run the app via the Electron binary (e.g. double‑click the .app or `npm run electron`), not with `node electron-main.cjs`. If you see **`Cannot find module '@solana/codecs-data-structures'`** (or similar under `@solana/`), run **`npm install`** from the project root so direct Solana codec deps are present, then rebuild the `.app` (`npm run build` or `./build-and-run.sh`). electron-builder’s prune can omit nested transitive packages unless they are listed in **`package.json` `dependencies`**.
...
**Heartbeat checklist file:** **`HEARTBEAT.md`** lives in the same **workspace** directory the server uses. **Electron:** `~/Library/Application Support/solagent/workspace/HEARTBEAT.md`. **`node server.js` from repo:** `agent/workspace/HEARTBEAT.md`. First-time app installs copy the template from the app bundle; keep repo and live workspace in sync if you customize. Set **`HEARTBEAT_INTERVAL_MS`** in Settings → Environment to enable periodic heartbeat **user messages** in the Chat view (see **heartbeat** tool note below). **`cronjob` task `heartbeat`** is only a server health log—it does **not** read `HEARTBEAT.md` or call the LLM.
...
- **heartbeat** – Returns a health-check payload (timestamp, status, memory, pid). Optional: set `HEARTBEAT_INTERVAL_MS` in Settings → Environment (or .env for testing). When set, the **server** logs a lightweight heap heartbeat on that interval; the **Electron Chat view** also posts the default heartbeat user prompt on the same interval so the agent runs **`HEARTBEAT.md`** checks in the open conversation (min 10s between ticks). Leave empty to disable.
