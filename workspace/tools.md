...
| **Core / web** | **browse** (SerpApi if key set), fetch_url, file_write, file_read, file_list, heartbeat, **cronjob** (includes **peg_monitor**), get_btc_price, **liquidity_regime_read** (Liquidity page → `memory/btc-liquidity-latest.json`), generate_image, analyze_image |
