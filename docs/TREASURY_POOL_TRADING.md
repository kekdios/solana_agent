...
| Layer | Use |
|--------|-----|
| **Pool reads** | Orca API (`https://api.orca.so/v2/solana/pools/{address}`) with on-chain RPC fallback; optional custom proxy via `orca_proxy_base_url`. |
...
- **Agent tools:** **`treasury_pool_info`** in `tools/treasury-pool-info.js` — read-only pool snapshot (Orca REST → on-chain decode fallback). **`treasury_pool_swap`** in `tools/treasury-pool-swap.js` — Whirlpool SDK execution only; both wired in **`server.js`** + **`config/tools.yaml`**.
