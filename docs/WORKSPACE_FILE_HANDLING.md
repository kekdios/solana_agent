...
**Conventional files:** Besides persona/rules (`SOUL.md`, `AGENTS.md`, `tools.md`, skills), the repo ships **`HEARTBEAT.md`** — an optional checklist the model follows when a **chat heartbeat** fires (see root **README.md** and **`HEARTBEAT_INTERVAL_SECONDS`** in Settings). Path is always under **`WORKSPACE_DIR`** (default: repo `workspace/` when running `node server.js`).
